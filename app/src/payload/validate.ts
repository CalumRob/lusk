  ...
  if (meta['indicator_pages'] !== undefined) {
    ...
    for (const [key, raw] of Object.entries(meta['indicator_pages'] as LigneBrute)) {
      ...
      if (family === 'relationship') {
        for (const role of ['axis', 'measure']) { const value = page[role] as LigneBrute; exiger(estObjet(value) && estChaine(value['indicator']) && indicator_keys.includes(value['indicator'] as string) && estChaine(value['label']) && estChaine(value['unit']), fichier, 0, `« indicator_pages.${key}.${role} » doit référencer un indicateur du thème et déclarer un rôle sémantique complet`); if (value && value['detail'] !== undefined && value['detail'] !== null) exiger(estObjet(meta['detail_labels']) && estObjet((meta['detail_labels'] as LigneBrute)[value['indicator'] as string]) && Object.prototype.hasOwnProperty.call((meta['detail_labels'] as LigneBrute)[value['indicator'] as string], String(value['detail'])), fichier, 0, `« indicator_pages.${key}.${role}.detail » est inconnu`) }
        const facet = page['scalarFacet'] as LigneBrute
        exiger(estObjet(facet) && estChaine(facet['indicator']) && indicator_keys.includes(facet['indicator'] as string) && (facet['direction'] === 'high' || facet['direction'] === 'low'), fichier, 0, `« indicator_pages.${key}.scalarFacet » doit référencer un indicateur publié et être explicite`); if (facet && facet['detail'] !== undefined && facet['detail'] !== null) exiger(estObjet(meta['detail_labels']) && estObjet((meta['detail_labels'] as LigneBrute)[facet['indicator'] as string]) && Object.prototype.hasOwnProperty.call((meta['detail_labels'] as LigneBrute)[facet['indicator'] as string], String(facet['detail'])), fichier, 0, `« indicator_pages.${key}.scalarFacet.detail » est inconnu`)
