  ...
  if (meta['indicator_pages'] !== undefined) {
    ...
    for (const [key, raw] of Object.entries(meta['indicator_pages'] as LigneBrute)) {
      ...
      for (const source of pageSources) { const record = (meta['source_records'] as LigneBrute)[source]; exiger(estObjet(record), fichier, 0, `source référencée « ${source} » introuvable`); for (const field of ['dataset', 'publisher', 'url', 'licence', 'vintage', 'freshness']) exiger(estChaine((record as LigneBrute)[field]) && ((record as LigneBrute)[field] as string).length > 0, fichier, 0, `source.${field} doit être renseignée`) }
       ...
       const rawSignature = page['signature'] as LigneBrute | undefined
       if (family === 'distribution') exiger(estObjet(rawSignature) && Array.isArray(rawSignature['details']) && (rawSignature['details'] as unknown[]).length > 0 && new Set(rawSignature['details'] as unknown[]).size === (rawSignature['details'] as unknown[]).length && (rawSignature['details'] as unknown[]).every((detail) => estChaine(detail) && Object.prototype.hasOwnProperty.call((meta['detail_labels'] as LigneBrute)[key], detail)), fichier, 0, `« indicator_pages.${key}.signature » est incomplète`)
       if (family === 'distribution') exiger(rawSignature?.['completeness'] === 'all-or-nothing', fichier, 0, `« indicator_pages.${key}.signature.completeness » doit être all-or-nothing`)
       ...
       indicator_pages[key] = { indicator: key, detail: detail === undefined ? null : detail as string | null, label: page['label'] as string, definition: page['definition'] as string, unit: page['unit'] as string, calculation: page['calculation'] as string, direction: page['direction'] as 'high' | 'low', caveats: page['caveats'] as string, levels: page['levels'] as ('commune' | 'epci' | 'departement')[], sources: pageSources, ...(page['family'] === undefined ? {} : { family: family as 'scalar' | 'distribution' }), ...(family === 'distribution' ? { summary, signature: { details: rawSignature!['details'] as string[], completeness: 'all-or-nothing' as const } } : {}) }
    }
  }
