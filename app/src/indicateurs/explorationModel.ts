...
export function modeleExploration(facts: readonly Indicateur[], metadata: ThemeMetadata, territoires: readonly Territoire[], requested: EtatExploration = {}, remembered?: string, indicator?: string): ModeleExploration {
  ...
  const signatureFacts = family === 'distribution' && requested.territoire ? facts.filter((fact) => fact.key === page.indicator && fact.type === niveau && fact.territoire === requested.territoire && fact.detail !== (summary?.detail ?? null)) : []
  const signatureComplete = Boolean(page.signature && page.signature.details.every((detail) => signatureFacts.some((fact) => fact.detail === detail && fact.value !== null)))
  const selectedSignature = family === 'distribution' && requested.territoire && signatureComplete ? page.signature!.details.map((detail) => ({ detail, value: signatureFacts.find((fact) => fact.detail === detail)!.value as number })) : null
  return { state: { niveau, departement: niveau === 'commune' ? requested.departement : undefined, epci: niveau === 'commune' ? requested.epci : undefined, territoire: requested.territoire, recherche: requested.recherche, tri, ordre }, rows, median, distribution: values, signature: selectedSignature, signatureState: family === 'distribution' && requested.territoire ? (signatureComplete ? 'complete' : 'unavailable') : null, summaryLabel: summary?.label ?? page.label, summaryUnit: summary?.unit ?? page.unit, density, high: { count: highRows.length, rows: highRows.length === 1 ? highRows : [] }, low: { count: lowRows.length, rows: lowRows.length === 1 ? lowRows : [] }, scopeLabel: requested.departement ? `Département ${requested.departement}` : requested.epci ? `EPCI ${requested.epci}` : 'Bretagne', direction: page.direction, markerX: positionDensite(density, selected), markerY: hauteurDensite(density, selected) }
}
