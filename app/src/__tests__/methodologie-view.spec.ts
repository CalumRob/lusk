...
describe('MethodologieView — la section « les sources »', () => {
  ...
  it('liste les 52 sources commises (l\u2019union est le contrat)', async () => {
    const { wrapper } = await monter(async () => payloadAvecVintages)
    ...
    // 53 depuis l'issue #243 : les HUIT archives OCS-GE millésimées remplacent
    // les quatre différentielles (49 + 8 − 4) — l'union est le contrat.
    expect(wrapper.findAll('section#sources tbody tr').length).toBe(53)
  })
...
describe('MethodologieView — la dégradation gracieuse', () => {
  it('vintages.json absent : les faits éditoriaux restent, la fraîcheur rend l\u2019état vide honnête', async () => {
    ...
    // La page ne casse jamais : 53 lignes dans la table des sources, fraîcheur
    // en tirets (les HUIT archives OCS-GE millésimées #243 remplacent les
    // quatre différentielles depuis la régénération réelle ; la table des
