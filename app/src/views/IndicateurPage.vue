...
const model = computed(() => page.value && metadata.value ? modeleExploration(facts.value, metadata.value, payload.value.territoires, requested.value, localStorage.getItem('lusk:niveau-indicateur') ?? undefined, indicator.value) : null) as ComputedRef<ModeleExploration>
...
const payloadCarte = computed(() => {
  ...
  return payloadPourCarte(payload.value, theme.value as Theme, indicator.value, niveau, departement, epci, metadata.value)
})
const vue = computed(() => route.query.vue === 'carte' || route.query.vue === 'indicateur' ? route.query.vue : 'reperes')
const couche = computed<Couche | null>(() => page.value ? ({ source: 'indicateur', clef: page.value.family === 'distribution' && page.value.summary ? page.value.summary.indicator : indicator.value, detail: page.value.family === 'distribution' && page.value.summary ? page.value.summary.detail : (page.value.detail ?? null), libelle: page.value.family === 'distribution' && page.value.summary ? page.value.summary.label : page.value.label, parDefaut: true, sousGroupe: null, storyKey: null }) : null)
...
<template>
  <section class="indicateur-page" :class="`theme-${theme}`" :style="themeVars">
    ...
    <template v-else>
       <header><p class="sur-titre">{{ metadata?.label }}</p><h1>{{ page.label }}</h1><p>{{ page.definition }}</p><p v-if="page.family === 'distribution'" class="facet-note">Repères comparés par : {{ model.summaryLabel }}</p></header>
       <section v-if="page.family === 'distribution'" class="distribution-grammar" aria-label="Signature et comparaison"><article><h2>Signature du territoire</h2><div v-if="model.signature" class="signature-bars" role="img" aria-label="Signature complète du territoire sélectionné"><span v-for="facet in model.signature" :key="facet.detail" :style="{ height: `${Math.max(2, facet.value * 100)}%` }" /></div><p v-else-if="model.signatureState === 'unavailable'" role="status">Signature indisponible : la distribution est incomplète ou supprimée.</p><p v-else>Sélectionnez un territoire pour voir sa distribution complète.</p></article><article><h2>Comparaison entre territoires</h2><p>Facette : <strong>{{ model.summaryLabel }}</strong> · {{ model.scopeLabel }}</p><svg class="comparison-plot" viewBox="0 0 600 120" role="img" aria-label="Comparaison des territoires sur la facette résumée"><path :d="`M ${model.density.map((point, index) => `${index * (600 / Math.max(model.density.length - 1, 1))},${20 + point.y}`).join(' L ')}`" /><circle v-if="model.markerX !== null" :cx="model.markerX * 6" cy="60" r="7" aria-label="Territoire sélectionné" /></svg></article></section>
...
<style scoped>
.distribution-grammar{display:grid;grid-template-columns:repeat(2,1fr);gap:16px;margin:16px 0}.distribution-grammar article{padding:20px;background:var(--surface-primary);border:1px solid var(--border-default);border-radius:12px}.signature-bars{height:100px;display:flex;align-items:end;gap:3px;border-bottom:2px solid var(--indicateur-line)}.signature-bars span{flex:1;background:var(--indicateur-accent);min-height:2px}.comparison-plot{width:100%;height:120px}.comparison-plot path{fill:none;stroke:var(--indicateur-accent);stroke-width:4}.comparison-plot circle{fill:var(--status-error)}
