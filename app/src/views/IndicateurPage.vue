...
import { modeleExploration, modeleRelation, payloadPourCarte } from '@/indicateurs/explorationModel'
import { niveauLePlusFin } from '@/indicateurs/explorationModel'
...
import RelationshipFigure from '@/components/fiche/RelationshipFigure.vue'
...
import type { Theme, RelationshipPageMetadata } from '@/payload/types'
...
const relation = computed(() => page.value?.family === 'relationship' ? page.value as RelationshipPageMetadata : null)
...
const model = computed(() => page.value && metadata.value && !relation.value ? modeleExploration(facts.value, metadata.value, payload.value.territoires, requested.value, localStorage.getItem('lusk:niveau-indicateur') ?? undefined, indicator.value) : null)
const relationLevel = computed(() => relation.value ? (niveauRoute.value && relation.value.levels.includes(niveauRoute.value) ? niveauRoute.value : niveauLePlusFin(relation.value.levels)) : 'commune')
const relationModel = computed(() => relation.value ? modeleRelation(payload.value.indicateurs.filter((fact) => fact.theme === theme.value), relation.value, payload.value.territoires, relationLevel.value, requested.value.territoire, theme.value as Theme) : null)
const themeVars = computed(() => themeValide.value ? themeStyle(theme.value as Theme) : undefined)
const directionGlyph = computed(() => (relation.value ? relation.value.scalarFacet.direction : page.value?.direction) === 'low' ? '▼' : '▲')
const directionText = computed(() => (relation.value ? relation.value.scalarFacet.direction : page.value?.direction) === 'low' ? 'moins = mieux' : 'plus = mieux')
const selectedRow = computed(() => model.value?.rows.find((row) => row.highlighted))
const markerDescription = computed(() => selectedRow.value && page.value ? `${selectedRow.value.territoire.nom} : ${formaterNombreFR(selectedRow.value.value, 2)} ${page.value.unit}, positionné sur l’axe de densité à sa valeur.` : '')
...
const payloadCarte = computed(() => {
  ...
  const mapIndicator = relation.value?.scalarFacet.indicator ?? indicator.value
  const mapDetail = relation.value?.scalarFacet.detail ?? null
  return payloadPourCarte(payload.value, theme.value as Theme, mapIndicator, niveau, departement, epci, mapDetail)
})
const vue = computed(() => route.query.vue === 'carte' || route.query.vue === 'indicateur' ? route.query.vue : 'reperes')
const couche = computed<Couche | null>(() => page.value ? ({ source: 'indicateur', clef: relation.value?.scalarFacet.indicator ?? indicator.value, detail: relation.value?.scalarFacet.detail ?? page.value.detail ?? null, libelle: relation.value ? metadata.value?.indicator_labels[relation.value.scalarFacet.indicator] ?? relation.value.label : page.value.label, parDefaut: true, sousGroupe: null, storyKey: null }) : null)
const activeLevel = computed(() => model.value?.state.niveau ?? relationModel.value?.level ?? relationLevel.value)
const niveauMasque = computed<NiveauMasque>(() => activeLevel.value === 'epci' ? 'epcis' : activeLevel.value === 'departement' ? 'departements' : 'communes')
const territoireCible = computed(() => payload.value.territoires.find((t) => t.territoire === route.query.territoire && t.type === activeLevel.value) ?? null)
...
<template>
  <section class="indicateur-page" :class="`theme-${theme}`" :style="themeVars">
    <div v-if="chargement" role="status">Chargement de l’indicateur…</div><div v-else-if="erreur" role="alert">Impossible de charger l’indicateur.</div><div v-else-if="!page || (!model && !relationModel)" role="alert">Indicateur introuvable.</div>
    <template v-else>
