...
const relationModel = computed(() => relation.value ? modeleRelation(payload.value.indicateurs.filter((fact) => fact.theme === theme.value), relation.value, payload.value.territoires, (niveauRoute.value ?? 'commune'), requested.value.territoire) : null)
...
<template>
  <section class="indicateur-page" :class="`theme-${theme}`" :style="themeVars">
    ...
    <template v-else>
      ...
      <main v-if="vue === 'reperes'" :class="{ 'relationship-page': relation }"><template v-if="relation && relationModel"><article class="relation-cloud"><h2>Repères des relations</h2><p>{{ relation.axis.label }} · {{ relation.measure.label }}</p><ul><li v-for="point in relationModel.points" :key="point.territoire.territoire" :class="{ selection: point.highlighted }"><RouterLink :to="point.fiche">{{ point.territoire.nom }}</RouterLink><span v-if="point.axis !== null && point.measure !== null"> · {{ point.axis }} {{ relation.axis.unit }} × {{ point.measure }} {{ relation.measure.unit }}</span><span v-else> · Données incomplètes</span></li></ul></article><article><h2>Tableau de comparaison</h2><p>Triée par {{ relationModel.scalarIndicator }} — la relation n’est pas un score.</p><RouterLink v-for="point in relationModel.table" :key="point.territoire.territoire" :to="point.fiche">{{ point.territoire.nom }} · {{ point.scalar }}</RouterLink></article></template><template v-else><div class="hero"><article class="median"><span>Médiane</span><strong>{{ model!.median === null ? '—' : formaterNombreFR(model!.median, 2) }} <small>{{ page.unit }}</small></strong><p>{{ model!.scopeLabel }}</p></article><article class="distribution"><h2>Distribution</h2><svg class="density" viewBox="0 0 600 180" role="img" aria-label="Densité des valeurs"><title>Densité des valeurs</title><desc v-if="markerDescription">{{ markerDescription }}</desc><path :d="`M ${model!.density.map((point, index) => `${index * (600 / Math.max(model!.density.length - 1, 1))},${20 + point.y * 1.5}`).join(' L ')}`" /><circle v-if="model!.markerX !== null && model!.markerY !== null" :cx="model!.markerX! * 6" :cy="20 + model!.markerY! * 1.5" r="7" class="point-highlight" :aria-label="markerDescription" /></svg><span v-if="markerDescription" class="visually-hidden">{{ markerDescription }}</span></article></div></template>
