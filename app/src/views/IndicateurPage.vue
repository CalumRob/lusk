  ...
  <section class="indicateur-page" :class="`theme-${theme}`" :style="themeVars">
    ...
    <template v-else>
      ...
      <nav class="vues" aria-label="Vues de l’indicateur"><button :class="{ active: vue === 'reperes' }" @click="setVue('reperes')">Repères</button><button :class="{ active: vue === 'carte' }" @click="setVue('carte')">Carte</button><button :class="{ active: vue === 'indicateur' }" @click="setVue('indicateur')">L’indicateur</button></nav>
       <main v-if="vue === 'reperes'" :data-family="profile ? 'profile' : 'scalar'"><template v-if="profile"><article class="profile"><h2>Profil complet</h2><p v-if="profile.state === 'empty'">Aucune valeur publiée pour ce profil.</p><p v-else-if="profile.state === 'short'">Profil partiel : certaines catégories ne sont pas disponibles.</p><p v-else-if="profile.state === 'unavailable'">Comparaison indisponible : les catégories ne sont pas publiées.</p><label v-if="profile.details.length">Facette comparée <select :value="profile.selected ?? ''" @change="setQuery('detail', ($event.target as HTMLSelectElement).value)"><option v-for="detail in profile.details" :key="detail" :value="detail">{{ metadata!.detail_labels[indicator]?.[detail] }}</option></select></label><ul><li v-for="row in profile.rows" :key="row.territoire?.territoire"><RouterLink v-if="row.territoire" :to="`/territoire/${row.territoire.type}/${row.territoire.territoire}?theme=${theme}`">{{ row.territoire.nom }}</RouterLink><span v-else>{{ row.label }}</span> · {{ formaterNombreFR(row.value, 2) }} {{ page.unit }}</li></ul></article></template><template v-else><div class="hero"><article class="median"><span>Médiane</span><strong>{{ model!.median === null ? '—' : formaterNombreFR(model!.median, 2) }} <small>{{ page.unit }}</small></strong><p>{{ model!.scopeLabel }}</p></article><article class="distribution"><h2>Distribution</h2><svg class="density" viewBox="0 0 600 180" role="img" aria-label="Densité des valeurs"><title>Densité des valeurs</title><desc v-if="markerDescription">{{ markerDescription }}</desc><path :d="`M ${model!.density.map((point, index) => `${index * (600 / Math.max(model!.density.length - 1, 1))},${20 + point.y * 1.5}`).join(' L ')}`" /><circle v-if="model!.markerX !== null && model!.markerY !== null" :cx="model!.markerX! * 6" :cy="20 + model!.markerY! * 1.5" r="7" class="point-highlight" :aria-label="markerDescription" /></svg><span v-if="markerDescription" class="visually-hidden">{{ markerDescription }}</span></article></div></template>
...
.profile{padding:24px;background:var(--surface-primary);border:1px solid var(--border-default);border-radius:12px}.profile ul{list-style:none;padding:0}.profile li{padding:10px 0;border-bottom:1px solid var(--border-subtle)}
</style>
