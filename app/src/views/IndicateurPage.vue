...
import type { NiveauIndicateur, OrdreExploration, TriExploration, TrajectoireLigne } from '@/indicateurs/explorationModel'
...
const requested = computed(() => ({ niveau: niveauRoute.value, ...validScope.value, territoire: typeof route.query.territoire === 'string' ? route.query.territoire : undefined, detail: typeof route.query.detail === 'string' ? route.query.detail : undefined, recherche: recherche.value, tri: ['nom', 'valeur', 'rang'].includes(String(route.query.tri)) ? route.query.tri as TriExploration : undefined, ordre: route.query.ordre === 'desc' ? 'desc' as OrdreExploration : 'asc' as OrdreExploration }))
...
const trajectoryYears = computed(() => [...new Set(model.value?.trajectoire?.flatMap((line) => line.points.flatMap((point) => point.year === null ? [] : [point.year])) ?? [])].sort((a, b) => a - b))
function trajectoryPath(line: TrajectoireLigne) {
  const years = trajectoryYears.value; if (!years.length) return ''
  const chunks: string[] = []; let current: string[] = []
  for (const point of line.points) {
    if (point.year === null || point.value === null) { if (current.length) chunks.push(current.join(' L ')); current = []; continue }
    const x = years.length === 1 ? 300 : 20 + ((point.year - years[0]) / (years.at(-1)! - years[0])) * 560
    const y = 200 - Math.max(-180, Math.min(180, point.value))
    current.push(`${x},${y}`)
  }
  if (current.length) chunks.push(current.join(' L ')); return chunks.map((chunk) => `M ${chunk}`).join(' ')
}
function trajectoryDescription(line: TrajectoireLigne) { return `${line.territoire.nom} : ${line.points.map((point) => `${point.label} ${point.value === null ? 'donnée manquante' : formaterNombreFR(point.value as number, 2)}`).join(', ')}` }
const markerDescription = computed(() => selectedRow.value && selectedRow.value.value !== null && page.value ? `${selectedRow.value.territoire.nom} : ${formaterNombreFR(selectedRow.value.value, 2)} ${page.value.unit}, positionné sur l’axe de densité à sa valeur.` : '')
function afficherRang(row: { rang: number | null; rangTaille: number }) { return row.rang === null ? '—' : `${row.rang === 1 ? '1er' : `${row.rang}e`} / ${row.rangTaille}` }
...
const payloadCarte = computed(() => {
  ...
  return payloadPourCarte(payload.value, theme.value as Theme, indicator.value, niveau, departement, epci, model.value?.family === 'trajectory' ? model.value.activeDetail : undefined)
})
const vue = computed(() => route.query.vue === 'carte' || route.query.vue === 'indicateur' ? route.query.vue : 'reperes')
const couche = computed<Couche | null>(() => page.value ? ({ source: 'indicateur', clef: indicator.value, detail: model.value?.family === 'trajectory' ? model.value.activeDetail ?? null : page.value.detail ?? null, libelle: page.value.label, parDefaut: true, sousGroupe: null, storyKey: null }) : null)
...
watch(() => [model.value?.activeDetail, route.query.detail], ([detail, queryDetail]) => { if (model.value?.family !== 'trajectory' || typeof detail !== 'string') return; if (queryDetail !== detail) router.replace({ query: normalizedQuery({ detail }) }) }, { immediate: true })
  ...
  <section class="indicateur-page" :class="`theme-${theme}`" :style="themeVars">
    ...
    <template v-else>
      ...
      <nav class="vues" aria-label="Vues de l’indicateur"><button :class="{ active: vue === 'reperes' }" @click="setVue('reperes')">Repères</button><button :class="{ active: vue === 'carte' }" @click="setVue('carte')">Carte</button><button :class="{ active: vue === 'indicateur' }" @click="setVue('indicateur')">L’indicateur</button></nav>
       <main v-if="vue === 'reperes'"><div v-if="model!.family === 'trajectory'" class="trajectory"><label>Endpoint actif <select :value="model!.activeDetail ?? ''" @change="setQuery('detail', ($event.target as HTMLSelectElement).value)"><option v-for="(label, detail) in page.endpoint_labels" :key="detail" :value="detail">{{ label }}</option></select></label><p class="trajectory-note">Endpoint actif : {{ page.endpoint_labels?.[model!.activeDetail ?? ''] ?? model!.activeDetail }}. Il pilote la carte, les extrêmes et le tableau. Le chemin complet reste visible.</p><svg viewBox="0 0 600 220" role="img" aria-label="Trajectoires complètes"><title>Trajectoires complètes</title><desc>Années alignées sur leur valeur réelle ; les données manquantes créent une rupture.</desc><path v-for="line in model!.trajectoire" :key="line.territoire.territoire" :class="{ 'trajectory-highlight': line.territoire.territoire === route.query.territoire }" :d="trajectoryPath(line)" :aria-label="trajectoryDescription(line)" /><text v-for="(year, index) in trajectoryYears" :key="year" :x="trajectoryYears.length === 1 ? 300 : 20 + (index / (trajectoryYears.length - 1)) * 560" y="216">{{ year }}</text></svg><ul class="visually-readable-trajectory"><li v-for="line in model!.trajectoire" :key="line.territoire.territoire"><strong>{{ line.territoire.nom }}</strong> — {{ trajectoryDescription(line) }}</li></ul></div><div class="hero"><article class="median"><span>Médiane · {{ model!.activeDetail ?? 'actif' }}</span><strong>{{ model!.median === null ? '—' : formaterNombreFR(model!.median, 2) }} <small>{{ page.unit }}</small></strong><p>{{ model!.scopeLabel }}</p></article><article class="distribution"><h2>{{ model!.family === 'trajectory' ? 'Spread de l’endpoint actif' : 'Distribution' }}</h2><svg class="density" viewBox="0 0 600 180" role="img" aria-label="Densité des valeurs"><title>Densité des valeurs</title><desc v-if="markerDescription">{{ markerDescription }}</desc><path :d="`M ${model!.density.map((point, index) => `${index * (600 / Math.max(model!.density.length - 1, 1))},${20 + point.y * 1.5}`).join(' L ')}`" /><circle v-if="model!.markerX !== null && model!.markerY !== null" :cx="model!.markerX! * 6" :cy="20 + model!.markerY! * 1.5" r="7" class="point-highlight" :aria-label="markerDescription" /></svg><span v-if="markerDescription" class="visually-hidden">{{ markerDescription }}</span></article></div>
