...
<template>
  <section class="indicateur-page" :class="`theme-${theme}`">
    ...
    <template v-else>
      ...
      <main v-if="vue === 'reperes'"><div class="hero"><article class="median"><span>Médiane</span><strong>{{ model!.median ?? '—' }} <small>{{ page.unit }}</small></strong><p>{{ model!.scopeLabel }}</p></article><article class="distribution"><h2>Distribution</h2><svg class="density" viewBox="0 0 600 180" role="img" aria-label="Densité des valeurs"><path :d="`M ${model!.density.map((point, index) => `${index * (600 / Math.max(model!.density.length - 1, 1))},${170 - Math.min(point.density * 20, 150)}`).join(' L ')}`" /><circle v-if="model!.markerX !== null" :cx="model!.markerX! * 6" cy="34" r="7" class="point-highlight" /></svg></article></div>
