...
# (#63). Le fixture est celui du câblage territorial OCS-GE (issue #237, amendé
# par #243) — sept communes, trois EPCIs, quatre départements et la région —
# pour que l'état par habitant existe et se classe. Depuis l'amendement #243,
# les états sont des STOCKS (le produit millésimé) : chaque commune a les DEUX
# états strictement positifs (plus aucune commune à M2 = 0 — le bug
# flux-et-état). Les valeurs vérifiées à la main (état M3 par habitant,
# m²/hab) — 35001 : 400/5200 · 56001 : 600/3100 · 29001 : 1200/2950 · 22001 :
# 1200/2400 · 22002 : 800/1300 · 29002 : 800/910 · 29003 : 700/500.
...
test_that("artif_par_habitant : classé TEL QUEL sur l'état à M3 (m²/hab, jamais une normalisation de surface)", {
  ...
  # les rangs de région (n = 7 communes à valeur) — les percentiles des
  # valeurs brutes M3 (0,0769 ; 0,1935 ; 0,4068 ; 0,5 ; 0,6154 ; 0,8791 ; 1,4),
  # vérifiés à la main : 35001 < 56001 < 29001 < 22001 < 22002 < 29002 < 29003
  expect_equal(valeur_payload(p, "35001", "artif_par_habitant", "2023")$rang_reg, 0)
  expect_equal(valeur_payload(p, "56001", "artif_par_habitant", "2024")$rang_reg,
               1 / 7)
  ...
  expect_equal(valeur_payload(p, "29002", "artif_par_habitant", "2024")$rang_reg,
               5 / 7)
  expect_equal(valeur_payload(p, "29003", "artif_par_habitant", "2024")$rang_reg,
               6 / 7)
  ...
  expect_equal(valeur_payload(p, "29002", "artif_par_habitant", "2024")$rang_epci, 1 / 3)
  expect_equal(valeur_payload(p, "29003", "artif_par_habitant", "2024")$rang_epci, 2 / 3)
  expect_equal(valeur_payload(p, "35001", "artif_par_habitant", "2023")$rang_epci, 0)
  expect_equal(valeur_payload(p, "56001", "artif_par_habitant", "2022")$rang_epci, 0.5)
...
test_that("artif_par_habitant : les DEUX lignes du territoire partagent le rang de l'état à M3", {
  ...
  expect_equal(m2$rang_reg, 1 / 7)

  # 22001 : M2 = 400/2200, M3 = 0,5 — même rang partagé (3/7 en région)
  ...
  # EPCI X (0,5405), EPCI Y (0,6193) et EPCI Z (0,1205) parmi les EPCIs de la
  # région (n = 3) ; les départements 22 (0,5405) / 29 (0,6193) / 35 (0,0769) /
  # 56 (0,1935) entre eux (n = 4)
  expect_equal(valeur_payload(p, "200000001", "artif_par_habitant", "2025")$rang_reg,
               1 / 3)
  expect_equal(valeur_payload(p, "200000002", "artif_par_habitant", "2024")$rang_reg,
               2 / 3)
  expect_equal(valeur_payload(p, "200000003", "artif_par_habitant", "M3")$rang_reg,
               0)
  expect_equal(valeur_payload(p, "22", "artif_par_habitant", "2025")$rang_reg, 2 / 4)
  expect_equal(valeur_payload(p, "29", "artif_par_habitant", "2024")$rang_reg, 3 / 4)
  expect_equal(valeur_payload(p, "35", "artif_par_habitant", "2023")$rang_reg, 0)
  expect_equal(valeur_payload(p, "56", "artif_par_habitant", "2024")$rang_reg, 1 / 4)
})
...
test_that("la série annuelle est inchangée : 14 lignes, le rang de la part de surface consommée", {
  ...
  # rangs de région (n = 6 communes à valeur) : 0 ; 1/6 ; 2/6 ; 0,5 ; 4/6 ; 5/6
...
test_that("un territoire à la série annuelle incomplète n'a pas de rang de série (l'état, lui, est là)", {
  ...
  # Depuis l'amendement #243, l'ÉTAT OCS-GE de 29003, lui, EXISTE (le produit
  # millésimé couvre tout le département) — le rang d'état est là, les DEUX
  # lignes publiées.
  annuel <- valeur_payload(p, "29003", "conso_enaf_annuel")
  expect_equal(nrow(annuel), 14L)
  expect_true(all(is.na(annuel$value)))
  expect_true(all(is.na(annuel$rang_reg)))
  e <- valeur_payload(p, "29003", "artif_par_habitant")
  expect_equal(nrow(e), 2L)
  expect_false(any(is.na(e$value)))
  expect_equal(unique(e$rang_reg), 6 / 7)
})
