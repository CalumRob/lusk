...
# courbe de rÃ©fÃ©rence mÃ©diane â€” calculÃ©s depuis la matrice temps FIGÃ‰E SEULE
# (+ les dÃ©nominateurs de population), sur le squelette COG 2025.
#
# Toute la sÃ©mantique est verrouillÃ©e sur un MICRO-FIXTURE calculÃ© Ã€ LA MAIN
# (quatre communes suffisent, testing decisions du parent #482) :
#   - A Â« 11111 Â» : deux points matrice (le sien + le village absorbÃ©
#     Â« 99999 Â», prÃ©-fusion) ;
#   - B Â« 22222 Â», C Â« 33333 Â» : un point chacune ;
#   - D Â« 44444 Â» : ABSENTE de la matrice (la commune non routÃ©e) â€” NA avec
#     motif nommÃ©, jamais un zÃ©ro silencieux.
...
# La matrice OD du micro-monde : p01 en minutes, une ligne par paire orientÃ©e.
...
# La projection des points matrice vers le COG 2025 (le village absorbÃ©
# Â« 99999 Â» tombe sous Â« 11111 Â») â€” la sortie de resoudre_codes_cog().
...
# L'appartenance EPCI/dÃ©partement du micro-monde : EPCI 200000001 = {A, B},
# EPCI 200000002 = {C} ; dÃ©partements 22 = {A, B}, 35 = {C}.
...
test_that("calculer_raccordement : les parts communales @90 sur le micro-fixture (Ã  la main)", {
  ...
  # A : elle-mÃªme (t = 0) + B Ã  60 min (min de 60/90 sur ses deux points) â€”
  # C n'arrive qu'Ã  100 min (min de 200/100) : hors du seuil 90
  expect_equal(part("11111"), 300 / 1000)
  # B : elle-mÃªme + C Ã  30 min + A Ã  70 min (le minimum de 70/80 sur les
  # deux points d'A passe le seuil)
  expect_equal(part("22222"), 600 / 1000)
  # C : elle-mÃªme + B Ã  40 min ; A n'arrive qu'Ã  150
  expect_equal(part("33333"), 500 / 1000)
  # D : NON ROUTÃ‰E â€” NA avec un motif nommÃ©, jamais un zÃ©ro silencieux
  ...
  # les communes routÃ©es n'ont pas de motif
...
test_that("calculer_raccordement : le minimum SUR LES POINTS gouverne (la fusion)", {
  # sans le village absorbÃ© Â« 99999 Â», C n'atteindrait A qu'Ã  200 min ; avec,
  # le minimum (100) passe le seuil 90 ? NON â€” 100 > 90 : la part de A reste
  # 0.3. En revanche la COURBE saute Ã  x = 100 exactement grÃ¢ce au point
  # absorbÃ© (sans lui, il faudrait attendre 200).
  calcul <- calculer_raccordement(
    fixture_matrice_micro(), fixture_population_micro(),
    fixture_codes_cog_micro()
  )
  ...
  expect_equal(lire(100), 600 / 1000)   # le point absorbÃ© ouvre la porte
  expect_equal(lire(90), 300 / 1000)
  # monotone croissante, plafonnÃ©e par W (hors D)
...
test_that("calculer_raccordement : la courbe cumulative, l'inclusion propre et l'inconnu", {
  ...
  # B seule Ã  t = 0 (l'inclusion propre), C la rejoint Ã  30, A Ã  70
  ...
  # la grille couvre [0, cap] au pas demandÃ© â€” 61 points de 0 Ã  600
  expect_setequal(unique(courbe_b$minute), seq(0, 600, by = 10))
  # la commune NON ROUTÃ‰E n'a pas de courbe (aucune valeur inventÃ©e)
  expect_false(any(calcul$courbes_communes$code == "44444"))
})

test_that("calculer_raccordement : les niveaux EPCI et dÃ©partement (l'union, jamais une moyenne)", {
  calcul <- calculer_raccordement(
    fixture_matrice_micro(), fixture_population_micro(),
    fixture_codes_cog_micro(), base_epci = fixture_base_epci_micro()
  )
  part <- function(table, code) table$part_90[table$code == code]
  # EPCI 200000001 {A, B} : on peut rejoindre L'EPCI en atteignant A OU B â€”
  # C n'y entre qu'Ã  min(100, 30) = 30 min. Part @90 = A + B + C = 0.6.
  # (Une moyenne pondÃ©rÃ©e donnerait (0.3Ã—100 + 0.4Ã—200)/300 â‰ˆ 0.367 â€” JAMAIS Ã§a.)
  expect_equal(part(calcul$epcis, "200000001"), 600 / 1000)
  # EPCI 200000002 {C} : elle-mÃªme + B Ã  30 ; A Ã  150 hors seuil
  expect_equal(part(calcul$epcis, "200000002"), 500 / 1000)
  # dÃ©partement 22 {A, B, D} : l'union + l'inclusion propre de D â€” le
  # territoire de D fait partie du dÃ©partement, chacun se rejoint soi-mÃªme :
  # A, B et D Ã  t = 0, C les rejoint Ã  30 min â†’ TOUTE la population
  expect_equal(part(calcul$departements, "22"), 1000 / 1000)
  # dÃ©partement 35 {C}
  expect_equal(part(calcul$departements, "35"), 500 / 1000)
  # rÃ©gion {A, B, C, D} : chacun se rejoint soi-mÃªme â€” routÃ©e ou non, tout
  ...
  # la courbe EPCI saute Ã  30 (l'union ouvre plus tÃ´t que chaque commune)
...
test_that("calculer_raccordement : la courbe de rÃ©fÃ©rence mÃ©diane bretonne", {
  calcul <- calculer_raccordement(
    fixture_matrice_micro(), fixture_population_micro(),
    fixture_codes_cog_micro()
  )
  ...
  # la mÃ©diane des TROIS communes routÃ©es (jamais la moyenne) :
  #   x = 0  -> parts (0.1, 0.2, 0.3)  -> mÃ©diane 0.2
...
test_that("resoudre_codes_cog : la projection MULTI-MILLÃ‰SIMES (un village prÃ©-2022)", {
  # la table COM rÃ©elle porte UNE ligne par commune 2025, les anciens codes
  # dans les colonnes CODGEO_<annÃ©e> : un code qui a quittÃ© le COG AVANT 2022
  # ne vit dans AUCUNE colonne 2022 â€” la rÃ©solution doit balayer TOUS les
  # millÃ©simes de la feuille.
  large <- tibble::tibble(
  ...
  # dÃ©terministe : l'ordre d'entrÃ©e conservÃ©
  expect_equal(names(codes), c("99999", "11111", "44444", "22222"))
})

test_that("TRIPWIRES resoudre_codes_cog : code inconnu et scission refusÃ©s bruyamment", {
  large <- tibble::tibble(
  ...
  # une scission (le mÃªme ancien code dans DEUX lignes) : jamais un choix
...
test_that("TRIPWIRE calculer_raccordement : un point matrice sans projection COG s'arrÃªte", {
...
# LA POPULATION Ã‰PINGLÃ‰E ---------------------------------------------------------
# Le dÃ©nominateur RP 2023 transcrit de la recherche vÃ©rifiÃ©e (W = 3 449 370),
# artefact de code Ã©pinglÃ© sous inst/extdata â€” le mÃªme pattern que la matrice
# (#485). Le contrat refuse tout substitut : empreinte recalculÃ©e SUR LE DISQUE.

test_that("la population Ã©pinglÃ©e passe son contrat (W verrouillÃ©)", {
...
test_that("TRIPWIRE â€” une population substituÃ©e (deux habitants Ã©changÃ©s) est refusÃ©e par l'empreinte", {
  source <- system.file("extdata", POPULATION_RACCORDEMENT_FICHIER,
                        package = "lusk")
  stopifnot(nzchar(source))
  faux <- lire_population_raccordement()
  # l'Ã©change garde la somme (le verrou W ne crie pas) : seule l'EMPREINTE
