# run_pipeline ----------------------------------------------------------------
# L'entrée unique. Point 9 : le tampon de fraîcheur du thème est pointé par
# l'id du manifeste (serie_historique), jamais par un sous-ensemble implicite.
# Point 11 : la composition des étapes (download -> construire -> vintage ->
# compute -> publish) est verrouillée par un test à étapes mockées — le réseau
...
test_that("run_pipeline compose les étapes dans l'ordre, à étapes mockées (point 11)", {
  ...
  # point 9 : le vintage du payload est la série historique, pointée par son id
  expect_equal(appels$vintage_vu$source, "INSEE — Série historique du recensement")
  expect_equal(appels$vintage_vu$version, "2023")
  expect_equal(appels$vintage_vu$date_reference, "2023-01-01")
  expect_equal(appels$vintage_vu$date_publication, "2026-06-30")
